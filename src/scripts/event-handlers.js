$(document).ready(function () {
  ...
  $(document).on("click", "#reset-button", function (event) {
    $("#alert-div").empty();
    /* If no operation has been made, the reset button can't reset anything
    *  Gives the user a notification */
    if (window.getLastOperationsLength() === 0) {
      ...
      /* Determines the old equation to generate a new rearrangement array */
  ...
  $(document).on("click", "#advice-button", function (event) {
    ...
    /* Checks if the alertDiv already includes an advice alert
    *  Removes that one if it's present */
    ...
    /* Adds a new alert, where the text is generated by the getAdviceMessage function */
    alertDiv.append(
      AlertTemplate({
